...
{
    public class GenerateCreditPayroll
    {
        public bool Execute()
        {
            ...
            try
            {
                ...
                //TaskFilter.TaskName.SearchString = "Submit Credit Pay Entry";
                //Task = ImageSolutions.Task.Task.GetTask(TaskFilter);

                string strPath = string.Format(@"C:\Import\CreditPayroll\{0}\", DateTime.UtcNow.ToString("yyyyMM"));
...
"
        ...
        protected void CreateExportExcelBySQL(string filepath, string countrycode, string prearningxrefcode, string sourcesystem)
        {
            ...
            if (countrycode == "US")
            {
                strFilename = string.Format("USA_PayEntryImport_ImageSolutions_Credit_{0}.xlsx", DateTime.UtcNow.ToString("yyyyMMddHHmmss"));
            }
            else if (countrycode == "CA")
            {
                strFilename = string.Format("CAN_PayEntryImport_ImageSolutions_Credit_{0}.xlsx", DateTime.UtcNow.ToString("yyyyMMddHHmmss"));
            }
            filepath = filepath + strFilename;
            ...
            try
            {
                ...
                EnterpriseFileSubmit.Type = "Credit Payroll Entry";
...
SELECT baa.BudgetAssignmentAdjustmentID, ec.WorkdayID as EmployeeID, Name.Value as Name, Comment.Value as Comment, ISNULL(baa.Amount,0) * -1 as Amount
FROM BudgetAssignmentAdjustment (NOLOCK) baa 
Inner Join BudgetAssignment (NOLOCK) ba on ba.BudgetAssignmentID = baa.BudgetAssignmentID 
Inner Join UserWebsite (NOLOCK) uw on uw.UserWebsiteID = ba.UserWebsiteID
...
Outer Apply
(
	SELECT Top 1 ab2.CountryCode
	FROM UserAccount (NOLOCK) ua2
	Inner JOin Account (NOLOCK) a2 on a2.AccountID = ua2.AccountID
	Inner JOIn AddressBook (NOLOCK) ab2 on ab2.AddressBookID = a2.DefaultShippingAddressBookID
	WHERE ua2.UserWebsiteID = uw.UserWebsiteID
) addr
WHERE uw.WebsiteID = {0}
AND baa.SubmitPayroll = 1
AND baa.IsPayrollSubmitted = 0
AND ISNULL(baa.Amount,0) != 0
AND ISNULL(addr.CountryCode,'US') = {1}
...
"
                ...
                while (objRead.Read())
                {
                    string strBudgetAssignmentAdjustmentID = Convert.ToString(objRead["BudgetAssignmentAdjustmentID"]);
                    ImageSolutions.Budget.BudgetAssignmentAdjustment BudgetAssignmentAdjustment = new ImageSolutions.Budget.BudgetAssignmentAdjustment(strBudgetAssignmentAdjustmentID);

                    BudgetAssignmentAdjustment.IsPayrollSubmitted = true;
                    BudgetAssignmentAdjustment.PayrollReference = EnterpriseFileSubmit.EnterpriseFileSubmitID;
                    BudgetAssignmentAdjustment.PayrollSubmittedOn = DateTime.UtcNow;
                    BudgetAssignmentAdjustment.Update(objConn, objTran);
