...
{
    public class DownloadPreEmployeeCustomer
    {
        public bool Execute()
        {
            ...
            try
            {
                string strLocalFolder = (ConfigurationManager.AppSettings["CustomerLocalPath"]) + DateTime.UtcNow.ToString("yyyyMM") + "\\EBA";
                ...
                {
                    _SftpClient.Connect();
                    SftpFiles = _SftpClient.ListDirectory(Convert.ToString(ConfigurationManager.AppSettings["CustomerPreEmployeeSFTPath"]));
                        ...
                        if (_SftpFile.FullName.Contains(".pgp"))
                        {
                            ...
                            if (CustomerImport == null)
                            {
                                using (Stream _Stream = File.OpenWrite(string.Format(@"{0}\{1}", strLocalFolder, _SftpFile.Name)))
                                {
                                    ...
                                    NewCustomerImport.FilePath = string.Format(@"{0}\EBA\{1}", DateTime.UtcNow.ToString("yyyyMM"), _SftpFile.Name);
                                    NewCustomerImport.FileName = _SftpFile.Name;
                                    NewCustomerImport.IsStore = false;
                                    NewCustomerImport.IsEncrypted = true;
                                    NewCustomerImport.IsPreEmployee = true;
                                    ...
                                    {
                                        _SftpClient.DeleteFile(string.Format(@"{0}/{1}", Convert.ToString(ConfigurationManager.AppSettings["CustomerPreEmployeeSFTPath"]), _SftpFile.Name));
                                    }
                                }
                            }
                        }
