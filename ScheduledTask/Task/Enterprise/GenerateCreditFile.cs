...
{
    public class GenerateCreditFile
    {
        public bool Execute()
        {
            try
            {
                string strPath = string.Format(@"C:\Import\Credit\{0}\", DateTime.UtcNow.ToString("yyyyMM"));
        ...
        protected void CreateExportCSVBySQL(string filepath, string suppliername, string suppliernumber, DateTime invoicedate)
        {
            string strFilename = string.Empty;

            strFilename = string.Format("Credit_ImageSolutions_{0}.csv", DateTime.UtcNow.ToString("yyyyMMddHHmmss"));
            ...
            try
            {
                ...
                EnterpriseFileSubmit.Type = "Credit";
                EnterpriseFileSubmit.Create(objConn, objTran);

                strInvoiceNumber = string.Format("CRED-{0}", EnterpriseFileSubmit.EnterpriseFileSubmitID);
                ...
                objReturn.Append(string.Format("Invoice,{0},{1},{2},,{3},YES,,,,No,,,,,30,,,,,{4},USD,,,,", strInvoiceNumber, suppliername, suppliernumber, invoicedate.ToString("MM/dd/yyyy"), "US - Tulsa Shared Services"));
                objReturn.AppendLine();

                strSQL = string.Format(@"

SELECT ec.EmployeeID, ec.FirstName, ec.LastName, s.SalesOrderID, s.NetSuiteInternalID, ISNULL(Location.PSGroup,'') as LocationPSGroup, ISNULL(Location2.ExternalID,'') as LocationExternalID
	, SUM(ISNULL(c.Total,0)) * -1 as Amount
FROM CreditMemo (NOLOCK) c
Inner Join SalesOrder (NOLOCK) s on s.SalesOrderID = c.SalesOrderID
...
and c.IsExported = 0
and (
    ISNULL(Location.PSGroup,'') not like 'A5%'
    and 
    ISNULL(Location.PSGroup,'') != 'A0066'
)
...
"
                ...
                while (objRead.Read())
                {
                    ...
                    List<ImageSolutions.CreditMemo.CreditMemo> CreditMemos = new List<ImageSolutions.CreditMemo.CreditMemo>();
                    ImageSolutions.CreditMemo.CreditMemoFilter CreditMemoFilter = new ImageSolutions.CreditMemo.CreditMemoFilter();
                    CreditMemoFilter.SalesOrderID = new Database.Filter.StringSearch.SearchFilter();
                    CreditMemoFilter.SalesOrderID.SearchString = strSalesOrderID;
                    CreditMemos = ImageSolutions.CreditMemo.CreditMemo.GetCreditMemos(CreditMemoFilter);

                    foreach (ImageSolutions.CreditMemo.CreditMemo _CreditMemo in CreditMemos)
                    {
                        _CreditMemo.IsExported = true;
                        _CreditMemo.EnterpriseFileSubmitID = EnterpriseFileSubmit.EnterpriseFileSubmitID;
                        _CreditMemo.Update(objConn, objTran);
                    }
                }
                ...
                if (IsIncomplete)
                {
                    ...
                    CreateExportCSVBySQL(strPath, suppliername, suppliernumber, DateTime.UtcNow);
                }
