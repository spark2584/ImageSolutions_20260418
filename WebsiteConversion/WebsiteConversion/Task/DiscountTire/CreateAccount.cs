...
namespace WebsiteConversion.Task.DiscountTire
{
    public class CreateAccount : NetSuiteBase
    {
        public bool Execute(string websiteid, string parentid, string createdby, string group, string customerinternalid, string searchid)
        {
            try
            {
                ...
                WebsiteGroupFilter.GroupName.SearchString = group;
                ...
                //AccountFilter.AccountName.SearchString = group;
                ...
                foreach (NetSuiteLibrary.com.netsuite.webservices.Customer _Customer in NSCustomers)
                {
                    try
                    {
                        //string strRegion = NetSuiteHelper.GetStringCustomFieldValue(_Customer, "custentity_is_dt_region").Trim();
                        string strRegion = Convert.ToString(NetSuiteHelper.GetStringCustomFieldValue(_Customer, "custentity_kotn_add_cust_id1"));
                        string strStore = Convert.ToString(NetSuiteHelper.GetStringCustomFieldValue(_Customer, "custentity_store_org_number"));
                        string strAddCustID3 = Convert.ToString(NetSuiteHelper.GetStringCustomFieldValue(_Customer, "custentity_kotn_add_cust_id3"));
                        ...
                        if (string.IsNullOrEmpty(strAddCustID3))
                        {
                            throw new Exception("Missing Additional Cust ID 3");
                        }
                        ...
                        ImageSolutions.Account.AccountFilter AccountFilter = new ImageSolutions.Account.AccountFilter();
                        AccountFilter.AccountName = new Database.Filter.StringSearch.SearchFilter();
                        AccountFilter.AccountName.SearchString = String.Format("{0} - {1}", strRegion, strStore);
                        AccountFilter.WebsiteID = new Database.Filter.StringSearch.SearchFilter();
                        AccountFilter.WebsiteID.SearchString = websiteid;
                        Account = ImageSolutions.Account.Account.GetAccount(AccountFilter);
                        if (Account == null)
                        {
                            ...
                            Account.CustomerInternalID = customerinternalid;
                            Account.ParentID = parentid;
                            Account.AccountName = String.Format("{0} - {1}", strRegion, strStore);
                            Account.DefaultWebsiteGroupID = WebsiteGroup.WebsiteGroupID;
                            Account.CreatedBy = createdby;
                            Account.Create();
                        }

                        string strPassword = strAddCustID3;
                        if (strAddCustID3.Length == 7)
                        {
                            strPassword = string.Format("0{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 6)
                        {
                            strPassword = string.Format("00{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 5)
                        {
                            strPassword = string.Format("000{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 4)
                        {
                            strPassword = string.Format("0000{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 3)
                        {
                            strPassword = string.Format("00000{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 2)
                        {
                            strPassword = string.Format("000000{0}", strAddCustID3);
                        }
                        else if (strAddCustID3.Length == 1)
                        {
                            strPassword = string.Format("0000000{0}", strAddCustID3);
                        }

                        //Create UserInfo
                        string strCompany = string.Empty;
                        switch (_Customer.parent.internalId)
                        {
                            case "3443499":
                                strCompany = "Discount Tire";
                                break;
                            case "3443500":
                                strCompany = "America's Tire";
                                break;
                            default:
                                throw new Exception("Unkown Parent");
                        }
                        ...
                        if (UserInfo == null)
                        {
                            ...
                            UserInfo.FirstName = strCompany;
                            UserInfo.LastName = strStore;
